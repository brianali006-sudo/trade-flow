...
async function callAI(prompt) {
  ...
  if (GROQ_KEY) {
    try {
      ...
      console.log('[GROQ] error ' + res.status + ': ' + res.body.substring(0, 500));
    } catch(e) {
...
app.get('/test-groq', async (req, res) => {
  if (!GROQ_KEY) return res.json({ error: 'No GROQ_API_KEY set' });
  try {
    const body = JSON.stringify({
      model: 'llama-3.1-8b-instant',
      max_tokens: 50,
    ...
    res.json({ status: r.status, body: JSON.parse(r.body) });
  } catch(e) {
